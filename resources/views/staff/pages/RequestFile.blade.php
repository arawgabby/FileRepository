...
<div class="grid grid-cols-1 md:grid-cols-2 gap-6 p-6">
    ...
    ">
                        ...
                        <td class="px-4 py-2 capitalize">
                            ...
                            <form action="{{ route('newFile-request.update-status', $request->id ?? $request->request_id) }}" method="POST" class="inline">
                                @csrf
                                <button name="action" value="approved" class="bg-green-500 text-white px-2 py-1 rounded ml-2 hover:bg-green-600" onclick="return confirm('Approve this request?')">Approve</button>
                                <button name="action" value="rejected" class="bg-red-500 text-white px-2 py-1 rounded ml-2 hover:bg-red-600" onclick="return confirm('Reject this request?')">Reject</button>
                            </form>
                            @endif
                        </td>
                    </tr>
                    @empty
                    <tr>
                        <td colspan="5" class="px-4 py-4 text-center text-gray-500">No requests for your files.</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
